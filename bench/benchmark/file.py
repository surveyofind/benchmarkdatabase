...
#     # Prepare the SQL query
#     cursor.execute("""
#         INSERT INTO benchmark_sbmdata (
#             sbm_type, longitude, latitude, district, tahsil, state, old_description,  gdc_username ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)""", values)
...
cursor.execute("SELECT keyid FROM benchmark_gcpdata ORDER BY keyid DESC LIMIT 1")
...
for index, row in df.iterrows():
    ...
    highest_number += 1
    new_keyid = f"GCP{highest_number:04d}"
    
    # Extract values from the row and handle None cases
    values = (
        new_keyid,
        row.get('gcp_name', None),
    ...
    # Prepare the SQL query
    cursor.execute("""
        INSERT INTO benchmark_gcpdata (
            keyid, gcp_name, ellipsoidheight, longitude, latitude, state, district, tahsil, old_description, pid, gdc_username
